...
export default function Page() {
  return (
    <div className="relative min-h-screen text-foreground px-4 md:px-10 py-16 max-w-5xl mx-auto overflow-x-hidden space-y-24">
      ...
      <HeroWidget title="DFS" number="2" hook={`Imagine exploring a maze with a piece of chalk. You follow one corridor as far as possible, then backtrack when you reach a dead end. DFS does the same thing on a graph: go deep first, then return.`} />
      <FirstIntuition mode="dfs" />
      <DeeperMechanics mode="dfs" />
      <NumberCrunchingLab mode="dfs" />
      <ThreeDSpace mode="dfs" />
      <RealWorldData mode="dfs" />
      <ClassicAI mode="dfs" />
      <DeepAI mode="dfs" />
      <ConceptMap mode="dfs" />
      <Footer mode="dfs" />
