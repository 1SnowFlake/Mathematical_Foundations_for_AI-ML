...
export default function Page() {
  return (
    <div className="relative min-h-screen text-foreground px-4 md:px-10 py-16 max-w-5xl mx-auto overflow-x-hidden space-y-24">
      ...
      <HeroWidget title="Dijkstra's Algorithm" number="3" hook={`Imagine a map where roads have different travel times. You would not choose the road with the fewest turns—you would keep the cheapest known total time. Dijkstra’s Algorithm repeatedly does exactly that.`} />
      <FirstIntuition mode="dijkstra" />
      <DeeperMechanics mode="dijkstra" />
      <NumberCrunchingLab mode="dijkstra" />
      <ThreeDSpace mode="dijkstra" />
      <RealWorldData mode="dijkstra" />
      <ClassicAI mode="dijkstra" />
      <DeepAI mode="dijkstra" />
      <ConceptMap mode="dijkstra" />
      <Footer mode="dijkstra" />
    </div>
  );
}
